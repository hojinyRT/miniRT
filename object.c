...
{	
    ...
    // printf("IN ! \n");
	sp = obj->element;
	oc = vec_sub(ray.orig, ((t_plane*)obj->element)->center);
	if (dis < 0)
		return (FALSE);
	sqrtd = sqrt(dis);
	root = (-half_b - sqrtd) / a;
	...
	// rec->t = root;
	// rec->p = ray_at(ray, root);
	// rec->normal = vec_div_double(vec_sub(rec->p, ((t_plane*)obj->element)->center), ((t_plane*)obj->element)->radius); // 정규화된 법선 벡터.
	// set_face_normal(ray, rec); // rec의 법선벡터와 광선의 방향벡터를 비교해서 앞면인지 뒷면인지 t_bool 값으로 저장.
	return (TRUE);
}
...
//             light_color = vec_add(light_color, point_light_get(info, lights.element));
//         lights = lights.next;
//     }
